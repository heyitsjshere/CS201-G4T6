"""
Load airport dataset into various tree data structures.
...
def load_airport_data_into_trees():
    """
    Load airport dataset into all tree structures.
    
    Returns:
        dict: Dictionary containing all tree structures
    """
    print("=" * 80)
    print("LOADING AIRPORT DATASET INTO TREE STRUCTURES")
    print("=" * 80)
    
    # Load cleaned airport data
    print("\nLoading airport data...")
    airport_df = load_cleaned_data('airport')
    print(f"Loaded {len(airport_df):,} airport records")
    ...
    records = [(row['overall_rating'], row.to_dict()) for _, row in airport_df.iterrows()]
    ...
    print("-" * 80)
    print(f"All {len(airport_df):,} records inserted into all trees")
    
    return trees, airport_df
...
def demonstrate_tree_operations(trees, df):
    ...
    print("\n[1] SEARCH: Find all records with rating 4.0")
    print("-" * 80)
    target_rating = 4.0
    ...
    print("\n[2] TOP-K: Get top 10 highest rated airports")
    print("-" * 80)
    k = 10
    ...
    for i, record in enumerate(top_k, 1):
        print(f"   {i}. {record['airport_name'][:40]:40} | Rating: {record['overall_rating']:.1f}")
    
    # Demo 3: Range search
    print("\n[3] RANGE: Find all records with rating between 3.0 and 4.0")
    print("-" * 80)
...
def main():
    """Main function to load and demonstrate airport tree structures."""
    try:
        # Load data into trees
        trees, df = load_airport_data_into_trees()
